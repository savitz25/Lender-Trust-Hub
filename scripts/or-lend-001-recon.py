#!/usr/bin/env python3
"""Prove the Oregon HMDA county vs LEI-cell application delta from committed files."""
...
COUNTY = ROOT / "data/hmda/by-state/OR/county_market_summary.csv"
...
check(len(county) == 36, "36 county rows")
check(county_apps == 146902, f"county apps {county_apps}")
check(county_orig == 89073, f"county orig {county_orig}")
check(county_den == 22290, f"county den {county_den}")
check(lei_apps == 145271, f"lei apps {lei_apps}")
check(lei_orig == 89073, f"lei orig {lei_orig}")
check(lei_den == 21731, f"lei den {lei_den}")
check(county_apps - lei_apps == 1631, "application delta")
check(county_orig - lei_orig == 0, "origination delta must stay 0")
check(county_den - lei_den == 559, "denial delta")
check(county_apps != lei_apps, "grains must remain distinct in source files")
print("or-lend-001-recon pass", {"page": county_apps, "lei_cells": lei_apps, "delta": county_apps - lei_apps})
