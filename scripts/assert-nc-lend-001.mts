...
assert.equal(parseLenderAsk('how many lenders are in North Carolina?').failClosedKind, 'nc-no-combined-lenders');
assert.equal(parseLenderAsk('mortgage lenders North Carolina').failClosedKind, 'nc-nccob-lender-class');
...
assert.equal(parseLenderAsk('mortgage complaints North Carolina').failClosedKind, 'nc-cfpb-observations');
...
assert.equal(parseLenderAsk('MOSR North Carolina').failClosedKind, 'nc-mosr-class');
assert.equal(parseLenderAsk('reverse mortgage lender North Carolina').failClosedKind, 'nc-reverse-class');
assert.equal(parseLenderAsk('NCHFA lenders').failClosedKind, 'nc-nchfa-program');
...
assert.equal(parseLenderAsk('mortgage enforcement docket North Carolina').failClosedKind, 'nc-nccob-orders');
assert.equal(parseLenderAsk('NCCOB license L-123456').failClosedKind, 'nc-nccob-license-identity');
...
assert.equal(s.crosswalks.EXACT_NCCOB_NMLS_SOURCE_NATIVE_PAIRS, 1380);
assert.equal(s.crosswalks.EXACT_NMLS_LEI_CROSSWALKS, 0);
assert.equal(s.crosswalks.EXACT_ENFORCEMENT_NCCOB_ATTACHMENTS, 0);
assert.equal(s.crosswalks.EXACT_ENFORCEMENT_NMLS_ATTACHMENTS, 0);
assert.equal(s.crosswalks.EXACT_CFPB_NMLS_ATTACHMENTS, 0);
assert.equal(s.nchfa.NC_NCHFA_PARTICIPATING_LENDER_ROWS, null);
assert.equal(s.hero.universe_value, 484454);
assert.notEqual(s.hero.universe_value, 1380);
assert.notEqual(s.hero.universe_value, 640);
