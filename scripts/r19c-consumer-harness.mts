...
// follow hasMore to the end with limit 1: every advertised page must be fetchable, and the end must be reached
{ let page = 1, more = true, fetched = 0; const keys = new Set<string>(); while (more && page <= 250) { const r = await call({ name: 'First', page, limit: 1 }); if (r.status !== 200) { ok(`limit 1: advertised page ${page} is fetchable`, false, r.status); break; } r.body.candidates.forEach((c) => keys.add(c.stableKey)); fetched += r.body.candidates.length; more = r.body.pagination!.hasMore; page += 1; } ok('limit 1: following hasMore reaches exactly total distinct records', fetched === p1.body.pagination!.total && keys.size === fetched, [fetched, keys.size, p1.body.pagination!.total]); const beyond = await call({ name: 'First', page: p1.body.pagination!.total + 1, limit: 1 }); ok('beyond-end page is empty and flagged, not a repeat', beyond.status === 200 && beyond.body.candidates.length === 0 && (beyond.body.pagination as unknown as { outOfRange: boolean }).outOfRange === true && beyond.body.pagination!.hasMore === false, beyond.body.pagination); }
...
for (const [label, body, status, state] of [['identifier refused', { name: 'NMLS 3030' }, 422, 'RESTRICTED_SCOPE'], ['malformed identifier attempt refused', { name: 'NMLS -3251' }, 422, 'RESTRICTED_SCOPE'], ['a person request finds nothing in the institution-only catalog', { name: 'Jane Doe loan officer' }, 200, 'NO_MATCH'], ['page past the window rejected', { name: 'First', page: 21, limit: 10 }, 400, 'INVALID_REQUEST'], ['unknown field rejected', { name: 'BMO Bank', url: 'https://evil.example' }, 400, 'INVALID_REQUEST'], ['bad page rejected', { name: 'BMO Bank', page: 0 }, 400, 'INVALID_REQUEST'], ['missing name rejected', {}, 400, 'INVALID_REQUEST']] as const) {
