...
try {
 ...
 const recover=p.getByRole('link',{name:'View the New Jersey denials total',exact:true});const recoveryHref=await recover.getAttribute('href');await recover.click();await p.waitForURL(origin+recoveryHref);await settled();assert.equal((await api()).result.countEvidence?.scope,'NJ');assert.equal((await api()).result.countEvidence?.action,'denial');
