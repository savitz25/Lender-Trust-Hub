#!/usr/bin/env python3
"""
Deepen Utah HMDA product coverage (high-volume phase).

  python scripts/build-hmda-utah-deepen.py

Source: data/hmda/by-state/UT/
Writes: data/hmda/utah/*_ut.csv + lei_to_nmls_mapping.csv
Patches: lib/hmda/states.ts UT majorCountySlugs
"""
...
SRC = ROOT / "data" / "hmda" / "by-state" / "UT"
OUT = ROOT / "data" / "hmda" / "utah"
...
UT_MAJORS: set[str] = set(UT_COUNTIES.keys())
...
NATIONAL_SLUG_BY_NMLS: dict[str, str] = {
    ...
    "666254": "cyprus-credit-union",
    "654272": "canyon-view-credit-union",
    "1591": "churchill-mortgage-nashville",
}

UT_CURATED_LEI: dict[str, dict[str, str]] = {
    # Nationals re-ID
    "549300HW662MN1WU8550": {
        ...
        "match_method": "ut_deepen_gleif_reidentify+public_nmls",
    },
    ...
    "549300MZ8VZJOVC63092": {
        ...
        "match_confidence": "high",
        "match_method": "ut_deepen_gleif+public_nmls",
    },
    ...
    "549300KJ8PAJ7E52HG32": {
        ...
        "match_confidence": "high",
        "match_method": "ut_deepen_gleif+directory_slug",
    },
    ...
    "5493000YNV8IX4VD3X12": {
        ...
        "match_confidence": "high",
        "match_method": "ut_deepen_gleif+silverton_dba",
    },
    ...
    # Newly linked high-value UT regionals
    "54930016ZSB1QWXLRJ13": {
        "institution_name_hmda": "MY MOVE MORTGAGE, LLC",
        "nmls_id": "",
        "our_lender_slug": "my-move-mortgage",
        "legal_name": "My Move Mortgage, LLC (dba Momentum Loans)",
        "match_confidence": "high",
        "match_method": "ut_deepen_gleif+ut_directory",
    },
    "549300HS714PZ0BMPN11": {
        "institution_name_hmda": "CYPRUS Federal Credit Union",
        "nmls_id": "666254",
        "our_lender_slug": "cyprus-credit-union",
        "legal_name": "Cyprus Credit Union",
        "match_confidence": "high",
        "match_method": "ut_deepen_gleif+public_nmls+ut_directory",
    },
    "5493000KQ2PZ8G6VNI80": {
        "institution_name_hmda": "CANYON VIEW",
        "nmls_id": "654272",
        "our_lender_slug": "canyon-view-credit-union",
        "legal_name": "Canyon View Credit Union",
        "match_confidence": "high",
        "match_method": "ut_deepen_gleif+public_nmls+ut_directory",
    },
    "549300IQGEUYRQBJC007": {
        "institution_name_hmda": "State Bank of Southern Utah",
        "nmls_id": "",
        "our_lender_slug": "state-bank-of-southern-utah",
        "legal_name": "State Bank of Southern Utah",
        "match_confidence": "high",
        "match_method": "ut_deepen_gleif+ut_directory",
    },
    "549300CM7HYHZ1MEJ624": {
        "institution_name_hmda": "Cache Valley Bank",
        "nmls_id": "",
        "our_lender_slug": "cache-valley-bank",
        "legal_name": "Cache Valley Bank",
        "match_confidence": "high",
        "match_method": "ut_deepen_gleif+ut_directory",
    },
    "549300JAEP0DFO79JO70": {
        "institution_name_hmda": "GRANITE FEDERAL CREDIT UNION",
        "nmls_id": "",
        "our_lender_slug": "granite-credit-union",
        "legal_name": "Granite Credit Union",
        "match_confidence": "high",
        "match_method": "ut_deepen_gleif+ut_directory",
    },
}
...
def fill_county(r: dict[str, str]) -> dict[str, str]:
    ...
    if not (r.get("county_name") or "").strip() and fips in UT_COUNTIES:
        return {**r, "county_name": UT_COUNTIES[fips]}
    return r
...
def load_prior_maps() -> dict[str, dict[str, str]]:
    ...
    for path in sorted((ROOT / "data" / "hmda").glob("*/lei_to_nmls_mapping.csv")):
        if "by-state" in str(path) or path.parent.name == "utah":
            continue
...
def patch_states_ts(major_slugs: list[str]) -> None:
    ...
    pattern = re.compile(
        r'(code: [\'"]UT[\'"],[\s\S]*?)majorCountySlugs: new Set\(\[[\s\S]*?\]\),',
        re.M,
    )
    m = pattern.search(text)
    if not m:
        raise SystemExit("Could not find UT majorCountySlugs in states.ts")
...
def main() -> None:
    ...
    for lei, cur in UT_CURATED_LEI.items():
    ...
    for r in county_rows:
        ...
        if not name and fips in UT_COUNTIES:
            r = {**r, "county_name": UT_COUNTIES[fips]}
            name = UT_COUNTIES[fips]
        if fips in UT_MAJORS or name or orig >= 200:
            if fips in UT_COUNTIES and not (r.get("county_name") or "").strip():
                r = {**r, "county_name": UT_COUNTIES[fips]}
            county_out.append(r)
    ...
    for r in read_csv(SRC / "lender_state_summary.csv"):
        ...
        if not (r.get("institution_name") or "").strip():
            ...
            if not nm and lei in UT_CURATED_LEI:
                nm = UT_CURATED_LEI[lei]["institution_name_hmda"]
            if nm:
    ...
    for r in cand:
        if "ut_originations" in r and col not in r:
            r[col] = r.get("ut_originations") or "0"
    ...
    def add_mapping(lei: str, base: dict[str, str], method_prefix: str) -> None:
        ...
        if method_prefix.startswith("ut_deepen") and curated_slug:
            slug = curated_slug
        else:
            slug = NATIONAL_SLUG_BY_NMLS.get(nmls) or curated_slug
        if not slug:
            return
        if not method_prefix.startswith("ut_deepen") and nmls in NATIONAL_SLUG_BY_NMLS:
        ...
        row = {
            ...
            "notes": (
                f"{method_prefix.rstrip('+') or 'curated'} map for UT activity "
                f"({st_orig} UT originations); slug={slug}."
            ),
        }
        prev = mapping_by_lei.get(lei)
        if prev and not method_prefix.startswith("ut_deepen"):
    ...
    if county_out:
        write_csv(OUT / "county_market_summary_ut.csv", county_out, list(county_out[0].keys()))
    if act_out:
        write_csv(OUT / "lender_activity_by_county_ut.csv", act_out, list(act_out[0].keys()))
    if state_rows:
        write_csv(OUT / "lender_state_summary_ut.csv", state_rows, list(state_rows[0].keys()))
    if cand:
        write_csv(OUT / "lei_mapping_candidates_ut.csv", cand, list(cand[0].keys()))
    if mapping_rows:
    ...
    major_named = [
        r
        for r in county_out
        if (r.get("county_fips") or "") in UT_MAJORS and (r.get("county_name") or "").strip()
    ]
    ...
    coverage = {
        ...
        "deferred_examples": [
            ...
            "Northpointe Bank",
            "Figure Lending",
            "Wayne / Piute / Daggett (thin volume)",
        ],
        "notes": (
            "Expanded major-county panels to 26; GLEIF re-ID; linked My Move Mortgage, "
            "Cyprus CU, Canyon View CU, State Bank of Southern Utah, Cache Valley Bank, "
            "Granite CU; retained America First / Mountain America / Intercap stack."
        ),
    }
