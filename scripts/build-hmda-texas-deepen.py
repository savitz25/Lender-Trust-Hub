#!/usr/bin/env python3
"""
Deepen Texas HMDA product coverage (high-volume phase).

  python scripts/build-hmda-texas-deepen.py

Source: data/hmda/by-state/TX/
Writes: data/hmda/texas/*_tx.csv + lei_to_nmls_mapping.csv
Patches: lib/hmda/states.ts TX majorCountySlugs

Rules:
- Exact / high-confidence LEI→NMLS/slug only
- GLEIF-aligned national re-identification overrides corrupted early FL/TX rows
- Prefer Texas directory hosts when a confident brand match exists
...
SRC = ROOT / "data" / "hmda" / "by-state" / "TX"
OUT = ROOT / "data" / "hmda" / "texas"
GLEIF_CACHE = ROOT / "data" / "hmda" / "florida" / "_gleif_name_cache.json"
STATE_TS = ROOT / "lib" / "hmda" / "states.ts"

# Full 5-digit FIPS → display name for major / named product markets
TX_COUNTIES: dict[str, str] = {
    "48201": "Harris",
    "48029": "Bexar",
    "48439": "Tarrant",
    "48085": "Collin",
...
# High-volume deepen band (panel majors) — first wave + next-band metros
TX_MAJORS: set[str] = {
    "48201",  # Harris
    "48029",  # Bexar
    "48439",  # Tarrant
...
# Prefer company hosts / TX regional directory slugs over out-of-state branch rows.
NATIONAL_SLUG_BY_NMLS: dict[str, str] = {
    ...
    "3274": "guild-mortgage-north-dfw",
    "3029": "crosscountry-mortgage-north-dfw",
    ...
    "1907": "veterans-united-north-texas",
    "1058": "lennar-mortgage-north-dfw",
    ...
    "1561": "silverton-mortgage-north-dfw",
    ...
    "583215": "rbfcu-spacex-corridor",
}

# GLEIF-verified LEI identities — always win over prior product maps.
TX_CURATED_LEI: dict[str, dict[str, str]] = {
    # ── National re-identify (fix corrupted early FL/TX identity swaps) ──
    "549300HW662MN1WU8550": {
        ...
        "match_method": "tx_deepen_gleif_reidentify+public_nmls",
    },
    ...
    "5493001SXWZ4OFP8Z903": {
        ...
        "match_confidence": "high",
        "match_method": "tx_deepen_gleif_reidentify+directory_slug",
    },
    "549300H3IZO24NSOO931": {
        "institution_name_hmda": "LENNAR MORTGAGE, LLC",
        "nmls_id": "1058",
        "our_lender_slug": "lennar-mortgage-north-dfw",
        "legal_name": "Lennar Mortgage, LLC",
        "match_confidence": "high",
        "match_method": "tx_deepen_gleif_reidentify+tx_directory",
    },
    ...
    "549300VZVN841I2ILS84": {
        ...
        "nmls_id": "3029",
        "our_lender_slug": "crosscountry-mortgage-north-dfw",
    ...
    "549300DD5QQUHO6PCH70": {
        ...
        "nmls_id": "1907",
        "our_lender_slug": "veterans-united-north-texas",
    ...
    "549300AQ3T62GXDU7D76": {
        ...
        "nmls_id": "3274",
        "our_lender_slug": "guild-mortgage-north-dfw",
    ...
    # ── Multi-state high-value lenders newly linked for TX volume ───────────
    "5493001GDRY0EL7VG372": {
        ...
        "match_confidence": "high",
        "match_method": "tx_deepen_gleif+public_nmls",
    },
    ...
    "5493000YNV8IX4VD3X12": {
        ...
        "nmls_id": "1561",
        "our_lender_slug": "silverton-mortgage-north-dfw",
        "legal_name": "Vanderbilt Mortgage and Finance, Inc. (dba Silverton Mortgage)",
        "match_confidence": "high",
        "match_method": "tx_deepen_gleif+tx_directory+silverton_dba",
    },
    ...
    # ── Texas regional with directory + NMLS ────────────────────────────────
    "549300LU6Y2TXG48QY48": {
        "institution_name_hmda": "RANDOLPH-BROOKS FEDERAL CREDIT UNION",
        "nmls_id": "583215",
        "our_lender_slug": "rbfcu-spacex-corridor",
        "legal_name": "Randolph-Brooks Federal Credit Union",
        "match_confidence": "high",
        "match_method": "tx_deepen_gleif+tx_directory_nmls",
    },
}
...
def fill_county(r: dict[str, str]) -> dict[str, str]:
    ...
    if not (r.get("county_name") or "").strip() and fips in TX_COUNTIES:
        return {**r, "county_name": TX_COUNTIES[fips]}
    return r
...
def load_prior_maps() -> dict[str, dict[str, str]]:
    ...
    for path in sorted((ROOT / "data" / "hmda").glob("*/lei_to_nmls_mapping.csv")):
        if "by-state" in str(path):
            continue
        # Skip current TX while rebuilding (stale identities)
        if path.parent.name == "texas":
            continue
...
def patch_states_ts(major_slugs: list[str]) -> None:
    ...
    # Support both single-quoted and double-quoted styles in file
    block_lines = [f"      '{s}'," for s in major_slugs]
    new_block = "majorCountySlugs: new Set([\n" + "\n".join(block_lines) + "\n    ]),"
    pattern = re.compile(
        r'(code: [\'"]TX[\'"],[\s\S]*?)majorCountySlugs: new Set\(\[[\s\S]*?\]\),',
        re.M,
    )
    m = pattern.search(text)
    if not m:
        raise SystemExit("Could not find TX majorCountySlugs in states.ts")
...
def main() -> None:
    ...
    for lei, cur in TX_CURATED_LEI.items():
    ...
    for r in county_rows:
        ...
        if not name and fips in TX_COUNTIES:
            r = {**r, "county_name": TX_COUNTIES[fips]}
            name = TX_COUNTIES[fips]
        # Keep named majors + other named rows + high-volume unnamed (named via table)
        if fips in TX_MAJORS or name or orig >= 2500:
            if fips in TX_COUNTIES and not name:
                r = {**r, "county_name": TX_COUNTIES[fips]}
            county_out.append(r)

    # ── State lender summary ────────────────────────────────────────────────
    state_rows = []
    for r in read_csv(SRC / "lender_state_summary.csv"):
        ...
        if not (r.get("institution_name") or "").strip():
            ...
            if not nm and lei in TX_CURATED_LEI:
                nm = TX_CURATED_LEI[lei]["institution_name_hmda"]
            if nm:
                r = {**r, "institution_name": nm}
        state_rows.append(r)

    # ── Lender activity in major counties ───────────────────────────────────
    act_out: list[dict[str, str]] = []
    for r in read_csv(SRC / "lender_activity_by_county.csv"):
        r = fill_county(r)
        fips = (r.get("county_fips") or "").strip()
        if fips not in TX_MAJORS:
            continue
        if not (r.get("county_name") or "").strip() and fips in TX_COUNTIES:
            r = {**r, "county_name": TX_COUNTIES[fips]}
        lei = (r.get("lei") or "").strip()
    ...
    for r in cand:
        if "tx_originations" in r and "texas_originations" not in r:
            r["texas_originations"] = r.get("tx_originations") or "0"
    ...
    def add_mapping(lei: str, base: dict[str, str], method_prefix: str) -> None:
        ...
        if method_prefix.startswith("tx_deepen") and curated_slug:
            slug = curated_slug
        else:
            slug = NATIONAL_SLUG_BY_NMLS.get(nmls) or curated_slug
        if not slug:
            return
        # Prefer NMLS host remap for non-curated reuse
        if not method_prefix.startswith("tx_deepen") and nmls in NATIONAL_SLUG_BY_NMLS:
        ...
        row = {
            ...
            "texas_originations": str(st_orig),
            ...
            "notes": (
                f"{method_prefix.rstrip('+') or 'curated'} map for TX activity "
                f"({st_orig} TX originations); slug={slug}."
            ),
        }
        prev = mapping_by_lei.get(lei)
        if prev and not method_prefix.startswith("tx_deepen"):
            # Keep higher-volume prior unless curated override
            if int(float(prev.get("texas_originations") or 0)) >= st_orig:
                return
    ...
    for lei, cur in TX_CURATED_LEI.items():
        add_mapping(lei, cur, "tx_deepen+")

    mapping_rows = sorted(
        mapping_by_lei.values(),
        key=lambda r: -int(float(r.get("texas_originations") or 0)),
    )

    # ── Write product files ─────────────────────────────────────────────────
    if county_out:
        write_csv(OUT / "county_market_summary_tx.csv", county_out, list(county_out[0].keys()))
    if act_out:
        write_csv(OUT / "lender_activity_by_county_tx.csv", act_out, list(act_out[0].keys()))
    if state_rows:
        write_csv(OUT / "lender_state_summary_tx.csv", state_rows, list(state_rows[0].keys()))
    if cand:
        write_csv(OUT / "lei_mapping_candidates_tx.csv", cand, list(cand[0].keys()))
    if mapping_rows:
    ...
    major_named = [
        r
        for r in county_out
        if (r.get("county_fips") or "") in TX_MAJORS and (r.get("county_name") or "").strip()
    ]
    ...
    # stable unique order by volume
    seen: set[str] = set()
    major_slugs_u: list[str] = []
    for s in major_slugs:
        if s and s not in seen:
            seen.add(s)
            major_slugs_u.append(s)

    patch_states_ts(major_slugs_u)

    mapped = {m["lei"] for m in mapping_rows}
    top20 = [r["lei"] for r in sorted(state_rows, key=lambda x: -float(x.get("total_originations") or 0))[:20]]
    top50 = [r["lei"] for r in sorted(state_rows, key=lambda x: -float(x.get("total_originations") or 0))[:50]]
    coverage = {
        "state": "TX",
        "phase": "texas-deepen",
        "major_counties": len(major_slugs_u),
        "major_county_slugs": major_slugs_u,
        ...
        "deferred_examples": [
            "Frost Bank (GLEIF known; NMLS/slug not force-linked)",
            "Prosperity Bank",
            ...
            "KBHS / Taylor Morrison / Jet HomeLoans / SWBC / Network Funding",
        ],
        "notes": (
            "Expanded major-county panels; rebuilt LEI maps with GLEIF re-ID; "
            "linked high-value multi-state + RBFCU/SSFCU/Benchmark/Kind/Cadence; "
            "low-confidence regionals intentionally deferred."
        ),
    }
    ...
    md = [
        ...
        f"- Top-20 mapped: **{coverage['top20_mapped']}/20** · Top-50 mapped: **{coverage['top50_mapped']}/50**\n\n",
