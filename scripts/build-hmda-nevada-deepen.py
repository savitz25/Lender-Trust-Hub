#!/usr/bin/env python3
"""
Deepen Nevada HMDA product coverage (high-volume phase).

  python scripts/build-hmda-nevada-deepen.py

Source: data/hmda/by-state/NV/
Writes: data/hmda/nevada/*_nv.csv + lei_to_nmls_mapping.csv
Patches: lib/hmda/states.ts NV majorCountySlugs
"""
...
SRC = ROOT / "data" / "hmda" / "by-state" / "NV"
OUT = ROOT / "data" / "hmda" / "nevada"
...
NV_COUNTIES: dict[str, str] = {
    ...
    "32001": "Churchill",
    "32013": "Humboldt",
    # Deepen — remaining counties with meaningful activity
    "32033": "White Pine",
    "32029": "Storey",
    "32015": "Lander",
    "32017": "Lincoln",
...
NV_MAJORS: set[str] = set(NV_COUNTIES.keys())
...
NATIONAL_SLUG_BY_NMLS: dict[str, str] = {
    ...
    "382004": "silver-state-schools-credit-union",
    "1591": "churchill-mortgage-nashville",
}

NV_CURATED_LEI: dict[str, dict[str, str]] = {
    # Nationals re-ID
    "549300FGXN1K3HLB1R50": {
        ...
        "match_method": "nv_deepen_gleif_reidentify+public_nmls",
    },
    ...
    "549300MZ8VZJOVC63092": {
        ...
        "match_confidence": "high",
        "match_method": "nv_deepen_gleif+public_nmls",
    },
    ...
    "5493000YNV8IX4VD3X12": {
        ...
        "match_confidence": "high",
        "match_method": "nv_deepen_gleif+silverton_dba",
    },
    ...
    "549300B4IYL7TZT8FA34": {
        ...
        "match_confidence": "high",
        "match_method": "nv_deepen_gleif+directory_slug",
    },
    ...
    # Existing NV / Mountain West hosts
    ...
    # Newly linked high-value NV regionals
    "5493002QI2ILHHZH8D20": {
        ...
        "match_confidence": "high",
        "match_method": "nv_deepen_gleif+nv_directory",
    },
    "549300WSQPVY1NRH6062": {
        "institution_name_hmda": "SILVER STATE SCHOOLS SERVICE COMPANY, LLC",
        "nmls_id": "382004",
        "our_lender_slug": "silver-state-schools-credit-union",
        "legal_name": "Silver State Schools Credit Union (service company LEI)",
        "match_confidence": "high",
        "match_method": "nv_deepen_gleif+public_nmls+nv_directory",
    },
    "549300PMVNBERP6TB157": {
        "institution_name_hmda": "ONE NEVADA CREDIT UNION",
        "nmls_id": "",
        "our_lender_slug": "one-nevada-credit-union",
        "legal_name": "One Nevada Credit Union",
        "match_confidence": "high",
        "match_method": "nv_deepen_gleif+nv_directory",
    },
}
...
def fill_county(r: dict[str, str]) -> dict[str, str]:
    ...
    if not (r.get("county_name") or "").strip() and fips in NV_COUNTIES:
        return {**r, "county_name": NV_COUNTIES[fips]}
    return r
...
def load_prior_maps() -> dict[str, dict[str, str]]:
    ...
    for path in sorted((ROOT / "data" / "hmda").glob("*/lei_to_nmls_mapping.csv")):
        if "by-state" in str(path) or path.parent.name == "nevada":
            continue
...
def patch_states_ts(major_slugs: list[str]) -> None:
    ...
    pattern = re.compile(
        r'(code: [\'"]NV[\'"],[\s\S]*?)majorCountySlugs: new Set\(\[[\s\S]*?\]\),',
        re.M,
    )
    m = pattern.search(text)
    if not m:
        raise SystemExit("Could not find NV majorCountySlugs in states.ts")
...
def main() -> None:
    ...
    for lei, cur in NV_CURATED_LEI.items():
    ...
    for r in county_rows:
        ...
        if not name and fips in NV_COUNTIES:
            r = {**r, "county_name": NV_COUNTIES[fips]}
            name = NV_COUNTIES[fips]
        # Keep majors + any named/high-volume rows
        if fips in NV_MAJORS or name or orig >= 50:
            if fips in NV_COUNTIES and not (r.get("county_name") or "").strip():
                r = {**r, "county_name": NV_COUNTIES[fips]}
            county_out.append(r)
    ...
    for r in read_csv(SRC / "lender_state_summary.csv"):
        ...
        if not (r.get("institution_name") or "").strip():
            ...
            if not nm and lei in NV_CURATED_LEI:
                nm = NV_CURATED_LEI[lei]["institution_name_hmda"]
            if nm:
    ...
    for r in cand:
        if "nv_originations" in r and col not in r:
            r[col] = r.get("nv_originations") or "0"
    ...
    def add_mapping(lei: str, base: dict[str, str], method_prefix: str) -> None:
        ...
        if method_prefix.startswith("nv_deepen") and curated_slug:
            slug = curated_slug
        else:
            slug = NATIONAL_SLUG_BY_NMLS.get(nmls) or curated_slug
        if not slug:
            return
        if not method_prefix.startswith("nv_deepen") and nmls in NATIONAL_SLUG_BY_NMLS:
        ...
        row = {
            ...
            "notes": (
                f"{method_prefix.rstrip('+') or 'curated'} map for NV activity "
                f"({st_orig} NV originations); slug={slug}."
            ),
        }
        prev = mapping_by_lei.get(lei)
        if prev and not method_prefix.startswith("nv_deepen"):
    ...
    if county_out:
        write_csv(OUT / "county_market_summary_nv.csv", county_out, list(county_out[0].keys()))
    if act_out:
        write_csv(OUT / "lender_activity_by_county_nv.csv", act_out, list(act_out[0].keys()))
    if state_rows:
        write_csv(OUT / "lender_state_summary_nv.csv", state_rows, list(state_rows[0].keys()))
    if cand:
        write_csv(OUT / "lei_mapping_candidates_nv.csv", cand, list(cand[0].keys()))
    if mapping_rows:
    ...
    major_named = [
        r
        for r in county_out
        if (r.get("county_fips") or "") in NV_MAJORS and (r.get("county_name") or "").strip()
    ]
    ...
    coverage = {
        ...
        "deferred_examples": [
            ...
            "Panorama Mortgage / Gold Star Mortgage",
            "Canopy Mortgage / GoodLeap",
            "East West Bank / Toll Brothers Mortgage",
            "Moria Development",
            "Esmeralda / Eureka (minimal HMDA volume)",
        ],
        "notes": (
            "Expanded major-county panels to 15 (nearly full active NV set); GLEIF re-ID; "
            "linked KBHS, Silver State Schools CU, One Nevada CU; retained Greater Nevada / "
            "America First / Mountain America."
        ),
    }
