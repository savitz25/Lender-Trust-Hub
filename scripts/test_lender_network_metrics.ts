...
function baseInput(over: Partial<LenderNetworkMetricsInput> = {}): LenderNetworkMetricsInput {
  return {
    ...
    vaHmdaApplications: 340304,
    vaHmdaOriginations: 202417,
    vaCfpbMortgageComplaints: 14563,
    ...
    ncHmdaApplications: 484454,
    ncHmdaOriginations: 279735,
    ncFdicInstitutions: 91,
...
describe('lender-network-metrics-v1 grain safety', () => {
  ...
  it('does not convert NJ RMLA or CA CRMLA missing universes to zero', () => {
    ...
    assert.ok(m.network.publishedStateIntelligencePaths.includes('/illinois'));
    assert.equal(m.illinois.hmdaApplications, 394488);
    assert.equal(m.illinois.liveLicensedCompanyUniverse, null);
