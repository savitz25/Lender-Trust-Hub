#!/usr/bin/env python3
"""Prove the North Carolina HMDA county vs LEI-cell application delta from committed files."""
...
COUNTY = ROOT / "data/hmda/by-state/NC/county_market_summary.csv"
...
check(len(county) == 100, "100 county rows")
check(county_apps == 484454, f"county apps {county_apps}")
check(county_orig == 279735, f"county orig {county_orig}")
check(county_den == 86923, f"county den {county_den}")
check(lei_apps == 478702, f"lei apps {lei_apps}")
check(lei_orig == 279735, f"lei orig {lei_orig}")
check(lei_den == 84978, f"lei den {lei_den}")
check(county_apps - lei_apps == 5752, "application delta")
check(county_orig - lei_orig == 0, "origination delta must stay 0")
check(county_den - lei_den == 1945, "denial delta")
check(county_apps != lei_apps, "grains must remain distinct in source files")
print("nc-lend-001-recon pass", {"page": county_apps, "lei_cells": lei_apps, "delta": county_apps - lei_apps})
