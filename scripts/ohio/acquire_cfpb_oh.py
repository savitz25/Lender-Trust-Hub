...
def main() -> None:
    ...
    for start, end in months:
        ...
        payload = get_json(
            f"https://www.consumerfinance.gov/data-research/consumer-complaints/search/api/v1/?{qs}"
        )
    ...
    if total is not None and len(ids) != int(total):
        raise SystemExit(f"CFPB slice {len(ids)} != API total {total}")
    report = {
        "retrievedAt": NOW,
    ...
    (OUT / "cfpb-2025-oh-mortgage-ids.json").write_text(
        json.dumps({"ids": ids, "meta": report}, indent=2) + "\n", encoding="utf-8"
    )
    (OUT / "cfpb-2025-oh-mortgage-rows.json").write_text(
        json.dumps(rows, indent=2) + "\n", encoding="utf-8"
    )
    print("wrote", len(ids), "ids", flush=True)
