...
export function analyzeLoanEstimate(inputs: LoanEstimateInputs): LoanEstimateAnalysis {
  ...
  if (!hmdaLender) {
    limitations.push(
      'No matched 2025 HMDA evidence was found for the selected lender (or no lender was selected) among the 42 product-state HMDA slices (FL–OK). Fee placement uses educational bands only.'
    );
  } else {
    limitations.push(
