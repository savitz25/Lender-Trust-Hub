...
export function analyzeLoanEstimateClient(
  ...
  if (!hmdaLender) {
    limitations.push(
      'No matched 2025 HMDA evidence for the selected lender (or none selected) among product-state HMDA slices (FL–ME including RI, VT, ME). Fee placement uses educational bands only.'
    );
  } else {
    limitations.push(
