...
export function analyzeLoanEstimateClient(
  ...
  if (!hmdaLender) {
    limitations.push(
      'No matched 2025 HMDA evidence for the selected lender (or none selected) among the 47 product-state HMDA slices (FL–WV). Fee placement uses educational bands only.'
    );
  } else {
    limitations.push(
