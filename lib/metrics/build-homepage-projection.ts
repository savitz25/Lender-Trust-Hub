...
export function buildLenderHomepageStateCards(metrics: LenderNetworkMetricsV1): HomepageStateCard[] { return [
  ...
  },
  {
    ...
    name: 'Ohio',
    href: '/ohio',
    regulators: 'DFI · NMLS · HMDA/FFIEC · FDIC · CFPB · OHFA',
    sourceClocks: [
      { label: 'HMDA vintage', sourceAsOf: '2025', retrievedAt: null },
      { label: 'CFPB 2025 complaints', sourceAsOf: OHIO_SNAPSHOT.cfpb.period, retrievedAt: OHIO_SNAPSHOT.cfpb.retrieved_at },
      { label: 'Current RMLA roster', sourceAsOf: null, retrievedAt: null },
    ],
    evidence: ['HMDA 2025 Ohio market activity', 'CFPB 2025 mortgage complaints', 'OHFA county participating-lender observations', 'DFI/NMLS search-only current RMLA registration'],
    identityNote: 'Current RMLA company registration is search-only. Company ≠ MLO. Ohio license ≠ NMLS ID. HMDA applications are not lenders. OHFA names are not exact companies.',
    limitation: 'No Columbus/Cleveland intelligence routes. Missing RMLA roster is not zero companies. Do not headline OHFA names or FDIC banks as Ohio lenders.',
    highlights: [
      { label: 'HMDA applications', value: fmt(OHIO_SNAPSHOT.hmda.applications), grain: '2025 applications' },
      { label: 'HMDA originations', value: fmt(OHIO_SNAPSHOT.hmda.originations), grain: '2025 originations' },
      { label: 'CFPB 2025 mortgage complaints', value: fmt(OHIO_SNAPSHOT.cfpb.OH_CFPB_2025_MORTGAGE_COMPLAINT_ROWS), grain: 'calendar-year complaint IDs' },
    ],
  },
]; }
...
export function buildLenderHomepageEvidenceInventory(metrics: LenderNetworkMetricsV1): HomepageEvidenceMeasure[] {
  ...
  return [
    ...
    measure({ key: 'oh_hmda_apps', label: 'Ohio HMDA 2025 applications', value: OHIO_SNAPSHOT.hmda.applications, family: 'MORTGAGE_MARKET_ACTIVITY', grain: 'county-grain HMDA application observation', entityClass: 'mortgage application', geography: 'Ohio property location', sourceSystem: 'CFPB HMDA', acceptedArtifact: 'lib/ohio-intelligence/accepted-snapshot.json', sourceAsOf: '2025', retrievedAt: OHIO_SNAPSHOT.hmda.retrieved_at, definition: 'Applications for properties in Ohio.', counts: 'HMDA applications, not lenders.', doesNotCount: 'Ohio RMLA companies, FDIC banks, OHFA names, or current NMLS licenses.', researchDestination: '/ohio', identityRule: 'Property geography is not headquarters or license jurisdiction.' }),
    measure({ key: 'oh_cfpb', label: 'Ohio CFPB 2025 mortgage complaints', value: OHIO_SNAPSHOT.cfpb.OH_CFPB_2025_MORTGAGE_COMPLAINT_ROWS, family: 'CONSUMER_COMPLAINTS', grain: 'CFPB mortgage complaint ID', entityClass: 'consumer complaint', geography: 'Ohio', sourceSystem: 'CFPB', acceptedArtifact: 'lib/ohio-intelligence/accepted-snapshot.json', sourceAsOf: OHIO_SNAPSHOT.cfpb.period, retrievedAt: OHIO_SNAPSHOT.cfpb.retrieved_at, definition: 'Calendar-year 2025 CFPB mortgage complaints with state OH.', counts: 'Complaint IDs, not findings or licensed companies.', doesNotCount: 'DFI enforcement or NMLS identities.', publicationStatus: 'PUBLIC', researchDestination: '/ohio', identityRule: 'Company name is not an NMLS identity.' }),
    measure({ key: 'oh_live_roster_unacquired', label: 'Ohio current RMLA company roster', value: null, family: 'INSTITUTION_IDENTITY_LICENSING', grain: 'potential current NMLS company registration', entityClass: 'Ohio RMLA company registration', geography: 'Ohio', sourceSystem: 'DFI / NMLS Consumer Access', acceptedArtifact: 'lib/ohio-intelligence/accepted-snapshot.json', sourceAsOf: 'SOURCE_NOT_ACQUIRED', generatedAt: OHIO_SNAPSHOT.generated_at, definition: 'No bulk current RMLA company roster was acquired. DFI eLicense directs current research to NMLS Consumer Access.', counts: 'No numeric evidence total; this is an explicit coverage limitation.', doesNotCount: 'Zero companies or that HMDA/OHFA/FDIC counts are that census.', publicationStatus: 'PUBLIC_LIMITATION', researchDestination: '/ohio', identityRule: 'Current verification requires DFI + NMLS Consumer Access. Ohio license ≠ NMLS ID.' }),
    measure({ key: 'state_pages', label: 'Published state intelligence pages', value: metrics.network.publishedStateIntelligencePages, family: 'PUBLIC_RESEARCH_SURFACES', grain: 'published state intelligence page', entityClass: 'public research surface', geography: 'FL, NJ, CA, TX, WA, AZ, CO, VA, NY, IL, OR, PA, NC, OH', sourceSystem: 'Accepted state publication contracts', acceptedArtifact: 'LENDER_HOMEPAGE_STATE_CARDS', sourceAsOf: 'varies by state and evidence family', generatedAt: generated, definition: 'Live state intelligence destinations in the homepage state model.', counts: 'Published state pages derived from the same model rendered below.', doesNotCount: 'States with only national search results or a rating of research depth.', researchDestination: '#states', identityRule: null }),
