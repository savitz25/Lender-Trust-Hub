...
function filterChips(q: string, query: LenderResearchQuery, overrides: AskUrlOverrides): AskFilterChip[] {
  ...
  return [
    ...
    chip('usda', 'USDA', { loanType: 'USDA' }, loan === 'USDA'),
...
function stampContract(result: AskExecution): AskExecution {
  return {
    ...result,
    contract: LENDER_ASK_CONTRACT,
    interpretation: [{ label: 'Contract', value: LENDER_ASK_CONTRACT }, ...result.interpretation],
    trace: result.trace ? { ...result.trace, contract: LENDER_ASK_CONTRACT } : result.trace,
  };
}
