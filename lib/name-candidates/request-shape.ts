 ...
 * "LEI Financial Group" and a 20-letter company name may still be names. Bare numeric-only
 * input is neither a name nor an inferred NMLS id; isBareNumericOnlyInput keeps it out of the
 * name catalog. The institution / person / branch boundary is enforced by the catalog projection
 * (institutions only), not by guessing from vocabulary.
 */
 ...
 */
/**
 * Pure unlabeled digits after trimming and collapsing whitespace.
 * "2229" matches. "NMLS 2229", "2229 Minnesota", "1st National Bank", and "Mortgage 1 LLC" do not.
 * This is not an identifier parse: bare digits stay unlabeled.
 */
export function isBareNumericOnlyInput(text: string): boolean {
