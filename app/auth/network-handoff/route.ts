...
function failRedirect(reason: string) {
  const failUrl = new URL(HUB_DEFAULT_PATH.lender, HUB_ORIGINS.lender);
...
export async function GET(request: Request) {
  ...
  const successUrl = new URL(destPath, HUB_ORIGINS.lender);
  ...
  console.info('[network-handoff/complete] session set on Lender', { path: destPath });
