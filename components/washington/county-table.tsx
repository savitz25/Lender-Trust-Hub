...
import { fmtInt, fmtPct } from '@/lib/washington-intelligence/snapshot';
...
export function WashingtonCountyTable({ counties }: { counties: County[] }) {
    ...
    <div className="mt-4">
      <div className="min-w-0 overflow-x-auto">
        <table className="w-full min-w-[44rem] text-left text-sm">
          <caption className="sr-only">
            2025 HMDA activity for Washington counties in this extract. Denial rate is not a quality
            ranking. County names are not links.
          </caption>
          <thead>
      ...
      {rows.length > 12 ? (
        <button
          ...
          onClick={() => setOpen((v) => !v)}
        >
          {open ? 'Show fewer counties' : `Show all ${rows.length} Washington counties in this extract`}
        </button>
      ) : null}
      <p className="mt-2 text-xs text-slate-500">
        County is the HMDA property-location county. This table is not a county page and is not a
        ranking. Washington has 39 counties; this extract includes all 39. A county is market
        context only.
      </p>
    </div>
  );
