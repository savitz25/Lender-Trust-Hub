...
import { fmtInt, fmtPct } from '@/lib/california-intelligence/snapshot';
...
export function CaliforniaCountyTable({ counties }: { counties: County[] }) {
    ...
    <div className="mt-4">
      <div className="min-w-0 overflow-x-auto">
        <table className="w-full min-w-[44rem] text-left text-sm">
          <caption className="sr-only">
            2025 HMDA activity for all 58 California counties. Denial rate is not a quality ranking.
      ...
      {rows.length > 12 ? (
        <button
          ...
          onClick={() => setOpen((v) => !v)}
        >
          {open ? 'Show fewer counties' : `Show all ${rows.length} California counties`}
        </button>
      ) : null}
