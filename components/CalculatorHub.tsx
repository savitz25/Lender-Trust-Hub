...
const calcLoaders: Record<EmbedCalcId, () => Promise<{ default: React.ComponentType }>> = {
 ...
 amortization: () => import('@/components/calculators/AmortizationPlanner'),
 compare: () => import('@/components/LoanTypeComparator'),
 'rent-vs-buy': () => import('@/components/calculators/RentVsBuyCalc'),
 heloc: () => import('@/components/calculators/HELOCCalc'),
 'down-payment': () => import('@/components/calculators/DownPaymentCalc'),
 rental: () => import('@/components/calculators/RentalCashFlowCalc'),
 dti: () => import('@/components/DTIAnalyzer'),
...
export function CalculatorHub({ defaultCalc }: { defaultCalc?: CalcId }) {
 ...
 key={calc.id}
 className={cn(
 'group flex flex-col rounded-xl border border-zinc-200 bg-white p-5 shadow-sm transition-all hover:border-emerald-300 hover:shadow-md',
 calc.featured &&
 'border-emerald-200 bg-gradient-to-b from-white to-emerald-50/40'
 )}
 >
 ...
 <article className="flex flex-col rounded-xl border border-dashed border-zinc-300 bg-zinc-50 p-5">
 <h3 className="text-lg font-bold text-[#0A2540]">Lender directory</h3>
 ...
 type="button"
 onClick={closeCalc}
 className="mb-4 inline-flex items-center gap-1 text-sm font-semibold text-emerald-700 hover:text-emerald-900"
 >
