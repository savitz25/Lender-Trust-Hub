...
import { fmtInt, fmtPct } from '@/lib/virginia-intelligence/snapshot';
...
export function VirginiaCountyTable({ counties }: { counties: County[] }) {
  ...
  return (
    <div className="mt-4">
      <div className="min-w-0 overflow-x-auto">
        <table className="w-full min-w-[44rem] text-left text-sm">
          <caption className="sr-only">
            2025 HMDA activity for Virginia counties and independent cities in this extract. Denial
            rate is not a quality ranking. Names are not links. No local intelligence routes.
          </caption>
          <thead>
      ...
      {rows.length > 12 ? (
        ...
        >
          {open ? 'Show fewer geographies' : `Show all ${rows.length} Virginia counties and cities`}
        </button>
      ) : null}
    </div>
  );
