...
const origin = process.env.LTH_ORIGIN || 'https://www.lendertrusthub.com';
...
async function measure(page) {
  return page.evaluate(() => {
    ...
    const bar = document.querySelector('[class*="AskNetwork"], body > div, header');
    const networkBar = [...document.querySelectorAll('div, nav')].find((el) =>
      /Ask Trust Hub network/i.test(el.textContent || '') && el.getBoundingClientRect().height < 80 && el.getBoundingClientRect().y < 80,
    );
    const logo = document.querySelector('header a, nav a[href="/"]');
    ...
    return {
      ...
      logo: r(logo),
      switchHub: r(sw),
      stackedChrome: (networkBar ? networkBar.getBoundingClientRect().height : 0) + (header ? header.getBoundingClientRect().height : 0),
    };
  });
}
...
try {
  ...
  const hh = Math.min((report.desktop1440.stackedChrome || 140) + 8, 180);
  await p.screenshot({
  ...
  const mh = Math.min((report.mobile390.stackedChrome || 140) + 8, 180);
  await mp.screenshot({
