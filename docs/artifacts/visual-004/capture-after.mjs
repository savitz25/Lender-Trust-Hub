...
const origin = process.env.LTH_ORIGIN || 'http://127.0.0.1:3014';
...
async function measure(page) {
  return page.evaluate(() => {
    ...
    const my = [...document.querySelectorAll('a')].find((a) => /My Lending/i.test(a.getAttribute('aria-label') || a.textContent || ''));
    const bar = /Ask Trust Hub network/i.test(document.body.innerText.slice(0, 500));
    const r = (el) => {
      if (!el) return null;
    ...
    return {
      ...
      menu: r(menu),
      myLending: r(my),
      stackedBarPresent: bar && document.querySelectorAll('header').length > 1,
