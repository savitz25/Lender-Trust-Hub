...
## Additional acquired files inspected

The repository also contains `data/hmda/by-state/NJ/lender_activity_by_county.csv` (7,549 county/LEI rows; raw SHA256 `465538ca272fa2c91d25c42add77d1226c2090ecc5ef95e26d79a4afab4b97f5`) with a denial field. It is not imported into the institution runtime catalog and no existing NJ county-to-state institution execution path was found. This ticket preserves the existing declared FL county-to-state path rather than adding a new NJ source/aggregation capability. The unavailable outcome refers specifically to the selected state-level institution file, not to an assertion that every acquired NJ raw file lacks denials. Onboarding and certifying that distinct county-LEI path is separately scoped; scalar recovery already uses its existing accepted aggregate contract.
